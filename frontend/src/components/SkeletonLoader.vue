  ...
  <div class="animate-pulse" :class="containerClass">
    ...
    <template v-if="variant === 'card'">
      <div class="flex flex-wrap justify-center gap-10">
        ...
        >
          ...
          <div class="h-5 bg-white/10 rounded w-3/4 mb-4" />
          <div class="flex justify-between items-center mb-4">
            <div class="h-5 bg-white/10 rounded w-1/4" />
            <div class="h-8 bg-white/10 rounded-full w-16" />
          </div>
          <div class="h-11 bg-white/10 rounded-xl w-full" />
        </div>
      </div>
    </template>
    ...
    <template v-else-if="variant === 'detail'">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-14">
        ...
        <div class="space-y-6">
          <div>
            <div class="h-8 bg-white/10 rounded w-3/4 mb-2" />
            <div class="h-4 bg-white/10 rounded w-1/3" />
          </div>
          <div class="flex gap-1">
            <div v-for="i in 5" :key="i" class="w-5 h-5 bg-white/10 rounded" />
          </div>
          <div class="h-8 bg-white/10 rounded w-1/3" />
          <div class="h-6 bg-white/10 rounded-full w-24" />
          <div class="space-y-2">
            <div class="h-4 bg-white/10 rounded w-1/2" />
            <div class="h-4 bg-white/10 rounded w-2/5" />
          </div>
          <div class="h-12 bg-white/10 rounded-xl w-full" />
          <div class="space-y-2 pt-2">
            <div class="h-4 bg-white/10 rounded w-2/3" />
            <div class="h-4 bg-white/10 rounded w-1/2" />
            <div class="h-4 bg-white/10 rounded w-3/5" />
          </div>
        </div>
      </div>
    </template>
        ...
        >
          <div class="flex justify-between items-center px-6 py-4 bg-white/5 border-b border-white/6">
            <div class="flex items-center gap-4">
              ...
              <div>
                <div class="h-3 bg-white/10 rounded w-8 mb-1" />
                <div class="h-4 bg-white/10 rounded w-24" />
              </div>
            </div>
            <div class="flex items-center gap-4">
              <div class="h-6 bg-white/10 rounded-full w-20" />
              <div class="h-6 bg-white/10 rounded w-16" />
            </div>
          </div>
          <div class="px-6 py-4 space-y-3">
            <div v-for="j in 2" :key="j" class="flex items-center gap-4">
              <div class="w-14 h-14 bg-white/10 rounded-xl shrink-0" />
              <div class="flex-1">
                <div class="h-4 bg-white/10 rounded w-1/2 mb-1" />
      ...
      <div class="bg-[#111111] rounded-2xl border border-white/6 overflow-hidden">
        ...
        <div v-for="i in count" :key="i" class="px-6 py-4 border-b border-white/4">
          <div class="flex gap-8 items-center">
            ...
            <div class="h-4 bg-white/10 rounded w-16" />
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="variant === 'section'">
      <div class="max-w-6xl mx-auto px-6 py-16">
        <div class="flex items-center justify-between mb-8">
          <div class="h-8 bg-white/10 rounded w-56" />
          <div class="h-5 bg-white/10 rounded w-24" />
        </div>
        <div class="flex justify-center gap-6">
          <div v-for="i in count" :key="i" class="w-60 h-72 bg-white/10 rounded-xl" />
        </div>
      </div>
    </template>
    ...
    <template v-else>
      <div class="space-y-4">
        <div v-for="i in count" :key="i">
          ...
          <div class="h-4 bg-white/10 rounded w-5/6 mb-2" />
